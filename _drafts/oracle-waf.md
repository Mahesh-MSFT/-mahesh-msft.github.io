...
6. Manual deployments are usually the cause of unplanned downtime. Oracle web front end deployment can result in workload downtime if not automated. **Azure Devops** allow developers to automate complete web front end deployment. Using CI/CD workflow provides zero-touch deployment experience improving overall operational management for Oracle workload.
7. Large Oracle database size can result in database backup operation taking long time to complete. **Azure Backup** uses snapshot technology to considerably reduce backup operation duration. By using Azure Backup in application-consistent configuration for backing up Oracle database, backup operations are quicker. Azure backup stores backup file in **Azure Recovery Services Vault** which provides features such as cross-region restore, RBAC protection, central monitoring, soft delete protection and many other features. Together, Azure Backup and Recovery Service Vault help streamline backup operations for Oracle database.
8. Over time, Oracle application components deployment as well as configuration regression, can result in sluggish performance, out of control costs, neglected security posture, etc. **Azure Advisor** helps to analyse the Oracle workload running in Azure and provide optimization recommendations. By using Azure Advisor, Oracle workloads benefit from reduced costs, increased performance, better security posture and other benefits.
9. Partners who develop and deploy applications running Oracle database, can face additional challenges in managing them for potentially millions of deployments in customer's Azure subscriptions. **Azure Managed Applications** helps Partners to have complete control over the lifecycle of an Oracle workload while restricting administrative access to workload resources to customers who own the Azure subscription. Customers can have full visibility onto Oracle resources running in their Azure subscription but no overhead of managing them.
...
1. Usage profile of Oracle workloads can vary by time of the day, day of the week, or week(s) in a month or year. Not adjusting compute profile in response to usage profile can result in paying higher costs for less utilization **Azure AutoScale** helps to scale up/down or in/out by using key metrics such as CPU or Memory. Oracle web front ends running on Azure VMSS can adjust it's instance counts based on usage profile and help save money.
1. Not having any financial governance and accountability cloud resources can surprise many customers running Oracle workloads at month-end when the bill arrives. **Azure Cost Management & Billing** provides governance for effective cost management. By using Azure Cost Management & Billing Oracle workloads benefit from accountability with budgets, cost allocation, chargebacks & continuous cost optimization.
1. Taking a short-term cost view on lon-term commitment to running Oracle workloads in Azure, can result in unnecessary additional costs. **Azure Reservation** helps customers to reserve their usage capacity for one or three years. Because, resource utilization is reserved over long-term, Microsoft can use this visibility and optimize costs which are passed on to customers through discount. Oracle workloads can benefit from Azure Reservation by getting substantial discounts on Azure VMs and Storage.
1. When Oracle workloads run from on-premise sites, they already use Software-Assurance (SA) enabled licensing scheme. If not carried into cloud, such licensing scheme can loose it's utility and value. **Azure Hybrid Benefit** helps to carry on-premise SA licensing from on-premise to Azure and reduce licensing costs significantly. Oracle workloads benefit from reduced licensing costs for Windows, RedHat and SUSE Linux subscriptions.
